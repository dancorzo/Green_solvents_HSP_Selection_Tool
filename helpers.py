...
def make_dash_table(selection, solv_hsp):
    """ Return a dash defintion of an HTML table from a Pandas dataframe. """

    df_subset = solv_hsp.loc[solv_hsp["Name"].isin(selection)]
    table = []
...
def _create_layout(layout_type, xlabel, ylabel, zlabel):
    ...
    if layout_type == "scatter3d":
        base_layout["scene"] = {
            "xaxis": _create_axis(axis_type="3d", title=xlabel), # range=[0,22]),
    ...
    data = [
        {
            ...
            "marker": {
                ...
                "colorbar": {"title": "Toxicity [LD50 mg/kg]"}, # Title of color bar
                "line": {"color": "#444"},
                "reversescale": True,
                "sizeref": 10,
                "sizemode": "diameter",
